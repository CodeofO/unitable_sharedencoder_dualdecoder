...
TORCHRUN = $(CONDA_ACTIVATE) && HYDRA_FULL_ERROR=1 torchrun --rdzv-backend=c10d --rdzv_endpoint localhost:0 --nnodes=1 --nproc_per_node=$(NGPU)
# TORCHRUN = $(CONDA_ACTIVATE) && CUDA_VISIBLE_DEVICES=0,1 HYDRA_FULL_ERROR=1 torchrun --rdzv-backend=c10d --rdzv_endpoint localhost:0 --nnodes=1 --nproc_per_node=$(NGPU)
TORCHRUN = $(CONDA_ACTIVATE) && CUDA_VISIBLE_DEVICES=0,1,2,3 HYDRA_FULL_ERROR=1 torchrun --rdzv-backend=c10d --rdzv_endpoint localhost:0 --nnodes=1 --nproc_per_node=$(NGPU)
...
> $(PIP) install -r requirements_IX_241009.txt
> $(PIP) install -e .
> touch $@
...
######################
# NGPU := 2  # number of gpus used in the experiments
NGPU := 4
