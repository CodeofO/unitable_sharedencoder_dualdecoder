...
def create_sequential_scheduler(optimizer, schedulers, milestones):
    
    """
    옵티마이저를 각 스케줄러 설정에 주입하여 SequentialLR을 생성하는 헬퍼 함수
    """
    # 각 스케줄러 설정(config)을 instantiate 하면서 optimizer를 전달
    schedulers = [instantiate(cfg, optimizer=optimizer) for cfg in schedulers]
    
    # 최종 SequentialLR 객체 생성
...
def fix_bounding_boxes(bboxes):
    ...
    for bbox in bboxes:
        ...
        # x 좌표 수정 (x1 < x2)
        if x1 >= x2:
            x2 = x1 + 1  # 최소한 x2가 x1보다 1 크게 수정

        # y 좌표 수정 (y1 < y2)
        if y1 >= y2:
            y2 = y1 + 1  # 최소한 y2가 y1보다 1 크게 수정

        fixed_bboxes.append([x1, y1, x2, y2])
