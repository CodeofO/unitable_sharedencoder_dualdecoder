...
def clean_html(html_content):
    """강화된 HTML 정규화"""
    # 기존 html, body, table 태그 제거
    html_content = re.sub(r'</?(?:html|body)>', '', html_content)
    html_content = re.sub(r'</?table[^>]*>', '', html_content)
    
    # 태그 내부 텍스트 정리
    html_content = re.sub(r'(</(?:td|th)>)[^<>]+(<?)', r'\1\2', html_content)
    
    # 잘못된 태그 수정 
    html_content = re.sub(r'<\s*/+\s*(?:th|td)\s*>', r'</td>', html_content)
    ...
    html_content = re.sub(r'(rowspan|colspan)\s*=\s*(["\']?\s*\d+\s*["\']?)', fix_span, html_content)
    
    # 빈 태그 처리
    html_content = re.sub(r'<(td|th)\s*></\1>', r'<\1>&nbsp;</\1>', html_content)
    
    # thead/tbody 구조 통일
    # thead/tbody 태그가 없는 경우에만 처리
    if not re.search(r'<thead>', html_content):
        rows = re.findall(r'<tr>.*?</tr>', html_content, re.DOTALL)
        if rows:
            thead = f"<thead>{rows[0]}</thead>"
            tbody = f"<tbody>{''.join(rows[1:])}</tbody>"
...
def calculate_teds(gt_original, pred_origial):

    gt_html_cleaned = clean_html(gt_original)
    pred_html_cleaned = clean_html(pred_origial)
    
    ignore_nodes = ['html', 'body', 'head', 'style', 'meta', 'title']
    teds = TEDS(structure_only=False, n_jobs=1, ignore_nodes=ignore_nodes)
    s_teds = TEDS(structure_only=True, n_jobs=1, ignore_nodes=ignore_nodes)

    teds_score = teds.evaluate(pred_html_cleaned, gt_html_cleaned)
    s_teds_score = s_teds.evaluate(pred_html_cleaned, gt_html_cleaned)

    return teds_score, s_teds_score